class FakeDSGrid
{
	...
	public FakeDSGrid(int _w, int _h, int _fill = 0)
	{
	...
	public string Pack(bool _add_newlines = false)
	{
		...
		for (int _y = 0; _y < Height; _y++)
		{
			for (int _x = 0; _x < Width; _x++)
				_packed += Grid[_x, _y].ToString("x").PadLeft(2, '0');
			if (_add_newlines)
				_packed += "\r\n";
		}
		return _packed;
	...
	public void CopyFrom(FakeDSGrid _src, int _src_x, int _src_y, int _src_w, int _src_h, int _dst_x, int _dst_y)
	{
		...
		for (int _x = 0; _x < _src_w; _x++)
		{
			...
			for (int _y = 0; _y < _src_h; _y++)
				Grid[_fin_dst_x, Math.Min(_dst_y + _y, Height - 1)] = _src.Grid[_fin_src_x, Math.Min(_src_y + _y, _src.Height - 1)];
		}
	}
}